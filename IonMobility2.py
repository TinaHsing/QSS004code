...
class i2cDac:
	...
	def DacInit(self, address):
		self.i2cbus.write_block_data(address,EEPROM_WITH_Normal,self.zero)
		
	def DacSet(self, address, value):
		dacout = int(value*I2CDAC_CONV_CONST)
		out = dacout.to_bytes(2, byteorder ="big")
		self.i2cbus.write_byte_data(address,out[0],out[1])

	def DacClose(self, address):
		 self.i2cbus.write_block_data(address,EEPROM_WITH_PD,self.zero)
